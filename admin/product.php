...
$filter_featured = isset($_GET['filter_featured']) ? $_GET['filter_featured'] : null;
$filter_featured = isset($_GET['filter_new']) ? $_GET['filter_new'] : null; // Tìm theo tính nổi bật của sản phẩm
...
foreach ($results as $result) 
{
	...
	// 	if($product_info['sale']!=0){
	// 		$tong = $product_info['price'] - (int)($product_info['price'] * ($product_info['sale']/100));
	// 		}
	// 		else
	// 		{
	// 			$tong = $product_info['price'];
	// 		}
	// 		$soluong = $product_info['soluong'] - $product_info['quantity'];
	...
	$sql1 = " 
		...
		";
	$product_info = db_row($sql1);


	$bien_sale = '<span class="badge badge-danger">'.$result['sale'].'%'.'</span>' ;
	$bien_price = $result['price'] - (int)($result['price']*($result['sale']/100));
	$biensoluong = $result['soluong'] - $product_info['quantity'];
	...
	$products[] = array(
		...
		'sale_text'   => ($result['sale']) ? $bien_sale : 'Không giảm giá',
		
		//'soluong_text' => ($result['soluong']) ? 
		...
		'new_text'   => $result['new'] ? '<span class="badge badge-success">new</span>' : "not new",
...
if (isset($_GET['filter_sale'])) 
{
     $url .= '&filter_sale=' . $_GET['filter_sale'];
}
