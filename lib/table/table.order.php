...
function dayMonthYear($day, $month, $years){
    if((int)$day == 1){
        if((int)$month == 1){
            $dayNew   = 31;
            $monthNew = 12;
            ...
            $monthNew = $month - 1;
            if((int)$monthNew == 2){
                if((((int)$years % 4 == 0) && ((int)$years % 100 != 0)) || ((int)$years % 400 == 0)){
                    $dayNew = 29;
                }
                else{
                    $dayNew = 28;
            ...
            else if((int)$monthNew == 3||(int)$monthNew == 5||(int)$monthNew == 7||(int)$monthNew == 8||(int)$monthNew == 10||(int)$monthNew == 12){
                $dayNew = 31;
            }
            else{
                $dayNew = 30;
    ...
    return $yearsNew ."-". $monthNew ."-". $dayNew;
}
function orderGetTotalWithTime($time)
{
	$sql = "
		SELECT
			SUM(total)
		FROM `order` WHERE `date_added` LIKE '$time%';
	";
