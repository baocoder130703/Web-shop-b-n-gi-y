...
function productAdd($data)
{
	...
	$sale            = (int)$data['sale'];		// giá bán (vnd)
	$soluong         = (int)$data['soluong'];		// giá bán (vnd)
...
// 	";
	...
	$sql = "
		SELECT 
            p.product_id, p.model, p.name, p.image, p.manufacturer_id, p.price,p.new,p.sale,p.soluong, p.sort_order, p.status, p.date_added, p.description, p.best_seller, p.featured, p.tag,
			m.name AS manufacturer,
		FROM `product` AS p
...
// 	";
	...
	$sql = "
		SELECT
            p.product_id, p.model, p.name, p.image, p.manufacturer_id, p.price, p.sale,p.new, p.soluong,p.sort_order, p.status, p.date_added, p.description, p.best_seller, p.featured, p.tag,
			m.name AS manufacturer
		FROM `product` AS p
		LEFT JOIN `manufacturer` AS m ON (p.manufacturer_id = m.manufacturer_id)
		WHERE p.product_id = '{$product_id}' AND p.status = '1'
...
function productGetTotal($data=array())
{
	...
	if (isset($data['filter_sale']) && !is_null($data['filter_sale']))
	{
		$sql .= " AND `sale` = '" . (int)$data['filter_sale'] . "'";
	}
